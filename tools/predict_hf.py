    ...
    for image_path in tqdm.tqdm(images_path):
        ...
        if result_hanfeng[8].size == 0: ## 需要根据个人的修改*************************
            ...
            if not isExists:
                ...
                print(tmp_path+' 创建成功')
            print('hf_geshu',result_hanfeng[8].shape[0])
            for j in range(result_hanfeng[8].shape[0]):
              print('j_wei', j)
            for j in range(result_hanfeng[8].shape[0]):
...
if __name__ == '__main__':
    root_dir = '/xiaopeng/Have_done_Dataset/Binary_classification/random_data/16bit_train/good/' #存放要检测的图片
    crop_flag = True
    checkpoint_file_hanfeng = '/xiaopeng/backup/object_detection/hf/epoch_5.pth'
    config_file_hangfeng = '/xiaopeng/backup/object_detection/hf/cascade_rcnn_hrnetv2p_w32_20e_coco.py'
